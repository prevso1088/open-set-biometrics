            ...
            for lab in label_set:
                for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                    for vie in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
                        ...
                        if data_in_use is None:  # use everything
                            seq_dirs = [osp.join(seq_path, seq_dir) for seq_dir in seq_dirs]
                            seqs_info_list.append([*seq_info, seq_dirs])
                        else:
                            if all(data in seq_dirs for data in data_in_use):
                                seq_dirs = [osp.join(seq_path, data) for data in data_in_use]
                                seqs_info_list.append([*seq_info, seq_dirs])
                            else:
                                msg_mgr.log_debug(
