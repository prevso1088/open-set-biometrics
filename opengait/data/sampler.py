...
class TripletSampler(tordata.sampler.Sampler):
    def __init__(self, dataset, batch_size, batch_shuffle=False):
        ...
        self.total_batch_size = self.batch_size[0] * self.batch_size[1]
    ...
    def __iter__(self):
        while True:
            sample_indices = []
            pid_list = sync_random_sample_list(self.dataset.label_set, k=len(self.dataset.label_set))
            for pid in pid_list:
                sequences_remaining = self.total_batch_size - len(sample_indices)
                if sequences_remaining <= 0:
                    break

                indices = self.dataset.indices_dict[pid]
                sequences_to_add = min(sequences_remaining, len(indices), self.batch_size[1])
                if sequences_to_add > 0:
                    indices = sync_random_sample_list(indices, k=sequences_to_add)
                    sample_indices += indices

            if self.batch_shuffle:
                sample_indices = sync_random_sample_list(sample_indices, len(sample_indices))

            total_size = int(math.ceil(self.total_batch_size / self.world_size)) * self.world_size
            sample_indices += sample_indices[:(self.total_batch_size - len(sample_indices))]
