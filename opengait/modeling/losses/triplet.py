...
class TripletLoss(BaseLoss):
    def __init__(self, margin, loss_term_weight=1.0, metric='euc', mining='batch_all'):
    ...
    def forward(self, embeddings, labels):
        ...
        dist_diff = self.Convert2Triplets(labels, ref_label, dist)  # [p, num_triplets]
    ...
    def ComputeDistance(self, x, y):
        ...
        """
        if self.metric == 'cos':
            return 1 - F.cosine_similarity(x.unsqueeze(2), y.unsqueeze(1), dim=-1)
    ...
    def Convert2Triplets(self, row_labels, clo_label, dist):
        ...
        triplets = []
        for i in range(n):
            ap_dist = dist[:, i, matches[:, i]].view(p, -1, 1)
            an_dist = dist[:, i, diffenc[:, i]].view(p, 1, -1)
            if self.mining == 'batch_hard':
                triplets.append(ap_dist.max(dim=1)[0] - an_dist.min(dim=2)[0])
            else:
                dist_diff = (ap_dist - an_dist).view(p, -1)
                triplets.append(dist_diff)
        triplets = torch.cat(triplets, dim=1)  # [p, num_triplets]
        return triplets
